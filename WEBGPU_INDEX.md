...
Raylib has been removed from source imports and the module/build/test dependency graphs. The old OpenGL renderer is no longer available. WebGPU's built-in font is ASCII-only. IME composition and live cross-monitor DPI behavior remain unverified.
...
All terrain, menu, inventory, container, vitals, lighting and mob previews use native WebGPU. Shared GPU screenshot fixtures live in `native_preview_fixture_windows_test.go`; the model workshop uses `mob_preview_windows.go` and `webgpu_workshop_windows.go`. The old smoke executable is a source-tree launcher for the native real-chunk preview. `renderer_dependency_test.go` prevents reintroducing Raylib imports or module entries.
