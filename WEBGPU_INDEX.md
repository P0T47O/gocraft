...
Local validation for the live path: `go test ./...`, `go vet ./...`, `go build .`, then `go run . -webgpu`. Preview tests are opt-in only and are no longer part of the normal iteration loop. This animation batch adds/moves Go symbols, so regenerate the symbol index after validation: `go run ./tools/codeindex -write` then `go run ./tools/codeindex -check`.
