...
Milestone A (runtime WebGPU renderer internals are Raylib-free) is complete, pending normal local compile/runtime validation. `TextureAtlas.UVs` now uses backend-neutral `AtlasRect`, so the WebGPU atlas no longer inherits `rl.Rectangle` from the legacy mesher. Milestone B has started: `Client.Update` now accepts primitive position/yaw/pitch values, so the TCP client no longer imports Raylib or depends on the gameplay `InputState`. Raylib remains for the native window/input owner, menus, the OpenGL reference renderer and shared gameplay/world math such as ray tests and collider vectors. The next dependency-removal slice is the world-ray / mob-attack boundary, where `rl.Ray`, `rl.Vector3` and `rl.BoundingBox` still leak into server-side gameplay.
