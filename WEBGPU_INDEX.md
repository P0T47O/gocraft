...
No new gameplay presentation is being added during the dependency-removal phase. The experimental viewmodel prototype was removed because the reference game did not have that feature. HUD, inventory, container, gameplay composition, native camera math and the core world renderer no longer import Raylib directly. `webgpu_game_windows.go` is now the temporary presentation boundary that snapshots screen size, time and camera data and supplies the HWND used to create the WebGPU surface. Mouse position is likewise funneled through that boundary. The obsolete standalone world `Draw` path and duplicate Raylib-camera scene/culling helpers have been removed.

Milestone A is now down to atlas/shared-type compatibility: `webgpu_atlas.go` still exposes `rl.Rectangle` because the legacy `TextureAtlas.UVs` and CPU mesher share that type. Raylib also remains for the native window/input owner, menus, the OpenGL reference renderer and several shared gameplay/server type leaks. The next dependency-removal step is to introduce a backend-neutral UV rectangle type at the shared atlas/mesher boundary, then narrow gameplay type leaks before replacing the window/input owner. The OpenGL/Raylib renderer stays as the reference path until existing feature parity is stable.
