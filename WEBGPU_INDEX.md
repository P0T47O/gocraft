...
No new gameplay presentation is being added during the dependency-removal phase. The experimental viewmodel prototype was removed because the reference game did not have that feature. HUD, inventory and container presentation no longer import Raylib directly; mouse position is funneled through the temporary Windows game-boundary bridge instead. The obsolete standalone `DrawWithHUD` compositor was removed so the gameplay path has one WebGPU presenter.

Raylib is still retained for the native window/input owner, camera/vector/rectangle types, menus, the OpenGL reference renderer, atlas UV compatibility with the legacy mesher, and several shared gameplay/server type leaks. The next removal steps are to move screen/time/camera data behind the same platform boundary, narrow the remaining shared type leaks, then replace the window/input owner and finally delete the OpenGL/Raylib renderer once the existing feature set reaches parity.
