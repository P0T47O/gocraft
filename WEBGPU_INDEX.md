...
No new gameplay presentation is being added during the dependency-removal phase. The experimental viewmodel prototype was removed because the reference game did not have that feature. HUD, inventory, container and the gameplay compositor no longer import Raylib directly. `webgpu_game_windows.go` snapshots screen size, time and camera data into a renderer-owned `webGPUFrameContext`; the gameplay path now uses `webGPUCamera` directly for projection, view, scene uniforms and frustum collection, so the temporary Raylib camera conversion has been removed. Mouse position is likewise funneled through the same boundary. The obsolete standalone `DrawWithHUD` compositor was removed so the gameplay path has one WebGPU presenter.

Raylib is still retained for the native window/input owner, legacy world-renderer constructor/standalone draw entry points, menus, the OpenGL reference renderer, atlas UV compatibility with the legacy mesher, and several shared gameplay/server type leaks. The next removal step is to delete or isolate those legacy world-renderer entry points so `webgpu_world_renderer_windows.go` itself stops importing Raylib, then narrow the remaining shared type leaks, replace the window/input owner and finally delete the OpenGL/Raylib renderer once the existing feature set reaches parity.
