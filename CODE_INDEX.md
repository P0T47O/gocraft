...
| 远景几何 LOD 原型：独立 Horizon 半径、按种子采样 128×128 格瓦片、8 格网格/水面、后台限流生成、WebGPU 有界上传和范围回收、完整区块环形交界；不创建远处完整 Chunk | [lod_terrain.go](lod_terrain.go)、[lod_renderer_windows.go](lod_renderer_windows.go)、[webgpu_gameplay_windows.go](webgpu_gameplay_windows.go)、[webgpu_camera_windows.go](webgpu_camera_windows.go)、[settings.go](settings.go)、[menu_screens.go](menu_screens.go)；CPU/实机预览回归 [lod_terrain_test.go](lod_terrain_test.go)、[lod_renderer_windows_test.go](lod_renderer_windows_test.go)；设计与限制 [LOD.md](LOD.md) |
