...
| WebGPU Playing 单 pass 合成（world + entities/effects + viewmodel + HUD/text + inventory/container） | [webgpu_gameplay_windows.go](webgpu_gameplay_windows.go)：`DrawGameplay` |
...
| WebGPU 第一人称手持物/viewmodel（共享 atlas、屏幕空间透视、挖掘摆动） | [webgpu_viewmodel_windows.go](webgpu_viewmodel_windows.go) |
...
- WebGPU 实机路径：Windows 下 `go run . -webgpu`，菜单仍由 Raylib/OpenGL 绘制，进入 Playing 后 WebGPU 独占 present；当前 world、实体/掉落物、JSON 生物模型与动画、挖掘裂纹、第一人称手持物、准星/热键栏/生命 HUD、ASCII 文本/聊天/debug/暂停死亡提示、创造/生存背包和箱子/熔炉容器已迁移；完整暂停菜单控件、特殊/动画非 atlas 材质仍待迁移。
...
工程仍使用 package main；WebGPU 实验现已接管 Playing 的 world、实体/effects、第一人称 viewmodel、主要 HUD/text、inventory/container presentation，但特殊材质，以及 window/input 与 Raylib 类型解耦仍未完成。