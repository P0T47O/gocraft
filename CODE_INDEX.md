...
| WebGPU Playing 双 pass 合成（4× MSAA 世界/实体解析到 surface；HUD/text、背包、暂停菜单以单采样绘制保持像素清晰） | [webgpu_gameplay_windows.go](webgpu_gameplay_windows.go)：`DrawGameplay`；资源与缩放 [webgpu_world_renderer_windows.go](webgpu_world_renderer_windows.go) |
...
| 爆炸事件：一次同步中心与有界方块移除列表，客户端应用后播放粒子和音效；拆掉门或支撑时将另一半门一并移除并纳入事件 | [protocol_explosion.go](protocol_explosion.go)、[explosion.go](explosion.go)、[door.go](door.go)、[game_packets.go](game_packets.go)、[game_explosion.go](game_explosion.go)、[webgpu_entities_windows.go](webgpu_entities_windows.go)、[game_audio.go](game_audio.go)；边界回归 [explosion_test.go](explosion_test.go) |
...
| 半砖/楼梯统一形状与朝向：橡木和石质变体、上下半块及双半砖合并/两份掉落、双半砖完整遮光、倒置楼梯和邻接转角、箱子/熔炉正面、服务端摆放朝向校验、局部网格/精确碰撞/射线/自动跨半格 | [block_shapes.go](block_shapes.go)、[shape_mesher.go](shape_mesher.go)、[chunk_mesher.go](chunk_mesher.go)、[world_ray.go](world_ray.go)、[world_light.go](world_light.go)、[actor_physics.go](actor_physics.go)、[player_movement.go](player_movement.go)、[input.go](input.go)、[server_packets.go](server_packets.go)、[block_registry.go](block_registry.go)、[recipes.go](recipes.go)；回归 [shaped_blocks_test.go](shaped_blocks_test.go) |
| 双格橡木门：支撑/空间校验、上半格同步、开关朝向与碰撞、拆除及爆炸成对清理 | [door.go](door.go)、[block_shapes.go](block_shapes.go)、[shape_mesher.go](shape_mesher.go)、[input.go](input.go)、[server_packets.go](server_packets.go)、[explosion.go](explosion.go)；区块边界/爆炸回归 [survival_features_test.go](survival_features_test.go)、[explosion_test.go](explosion_test.go) |
...
| 远景几何 LOD 原型：独立 Horizon 半径、按种子采样 128×128 瓦片、8 格地表/水面、与完整世界生成共用树木锚点的简化树冠（哈希预筛减少地形采样）；客户端区块快照/编辑的稀疏采样覆盖、瓦片版本失效和后台重建（未访问过的远处改动仍不可见）；完整区块加载期间的近景下垫层与渐变交界、WebGPU 有界上传和范围回收；不创建远处完整 Chunk | [lod_terrain.go](lod_terrain.go)、[lod_state.go](lod_state.go)、[generation_trees.go](generation_trees.go)、[lod_renderer_windows.go](lod_renderer_windows.go)、[world_packets.go](world_packets.go)、[world_core.go](world_core.go)、[world_pool.go](world_pool.go)、[game_packets.go](game_packets.go)、[webgpu_gameplay_windows.go](webgpu_gameplay_windows.go)、[webgpu_camera_windows.go](webgpu_camera_windows.go)、[settings.go](settings.go)、[menu_screens.go](menu_screens.go)；CPU/实机预览回归 [lod_terrain_test.go](lod_terrain_test.go)、[lod_state_test.go](lod_state_test.go)、[lod_renderer_windows_test.go](lod_renderer_windows_test.go)；设计与限制 [LOD.md](LOD.md) |
...
| WebGPU mipmap/AF 设置联动、远景轻微负 LOD bias、动画完整留白和 mip 更新、独立 UI 最近邻采样 | [texture_policy.go](texture_policy.go)、[webgpu_mipmap.go](webgpu_mipmap.go)、[webgpu_filter_windows.go](webgpu_filter_windows.go)、[webgpu_world_renderer_windows.go](webgpu_world_renderer_windows.go)、[webgpu_animation_windows.go](webgpu_animation_windows.go) |
...
| WebGPU 4× MSAA 世界几何、单采样 UI、菜单切换/尺寸重建实机回归（`GOCRAFT_WEBGPU_MSAA_PREVIEW=1`）与固定镜头过滤器对比（`GOCRAFT_WEBGPU_FILTER_DIAG=1`） | [webgpu_msaa_windows_test.go](webgpu_msaa_windows_test.go)、[webgpu_shapes_preview_windows_test.go](webgpu_shapes_preview_windows_test.go)、[menu_preview_webgpu_windows_test.go](menu_preview_webgpu_windows_test.go)；设置 [settings.go](settings.go)、[menu_screens.go](menu_screens.go) |
...
| 正六边形投影的三面方块图标、半砖/楼梯形状图标与门专用物品贴图、当前位置草/树叶/水着色、草侧覆盖层与独立物品精灵；实际 GPU 造型画廊 `GOCRAFT_WEBGPU_SHAPES_PREVIEW=1` | [webgpu_hud_windows.go](webgpu_hud_windows.go)、[item_registry.go](item_registry.go)、[webgpu_shapes_preview_windows_test.go](webgpu_shapes_preview_windows_test.go)、[webgpu_inventory_windows.go](webgpu_inventory_windows.go)、[mesh_tint.go](mesh_tint.go)、[webgpu_atlas.go](webgpu_atlas.go)、[webgpu_world_renderer_windows.go](webgpu_world_renderer_windows.go) |
...
| 剪羊毛与箭材料链：剪刀耐久、剪毛冷却/重长及外观状态、砂砾随机燧石、鸡掉羽毛、工作台箭配方 | [mob_breeding.go](mob_breeding.go)、[mob_protocol.go](mob_protocol.go)、[mob_server.go](mob_server.go)、[content/entities/chicken.json](content/entities/chicken.json)、[content/models/chicken.json](content/models/chicken.json)、[server_packets.go](server_packets.go)、[recipes.go](recipes.go)、[item_registry.go](item_registry.go)、[webgpu_entities_windows.go](webgpu_entities_windows.go)；回归 [survival_features_test.go](survival_features_test.go) |
