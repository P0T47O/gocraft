...
// DrawWithHUD is the gameplay presentation path. It mirrors the world renderer
// pass but adds the native WebGPU HUD before presenting, so gameplay no longer
// depends on raylib drawing for the crosshair, hotbar, or survival vitals.
func (r *webGPUWorldRenderer) DrawWithHUD(world *World, camera rl.Camera3D, state *InputState) error {
	if world == nil {
		return nil
	}
